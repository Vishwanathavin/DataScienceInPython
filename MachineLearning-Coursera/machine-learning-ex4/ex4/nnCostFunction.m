...
J = sum(sum((-log(a3).*y_output)- (log(1-a3).*(1-y_output))))/m + (lambda/(2*m))*(sum(sum(Theta1(:,2:size(Theta1,2)).^2)) + sum(sum(Theta2(:,2:size(Theta2,2)).^2)));
...
delta_3 = a3(t,:) - y_output(t,:);


delta_2 = zeros(hidden_layer_size);

delta_2 = (Theta2(:,2:size(Theta2,2))'*delta_3') .* (sigmoidGradient(z2(t,:))');
